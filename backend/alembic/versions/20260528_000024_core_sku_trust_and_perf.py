"""Core SKU trust view enrichment and performance indexes.

Revision ID: 20260528_000024
Revises: 20260520_000023
...
revision = "20260528_000024"
down_revision = "20260520_000023"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        SELECT
            ...
            CASE
                ...
                WHEN (
                    LOWER(COALESCE(cost.cost_source, '')) = 'manual_upload'
                    AND cost.upload_id IS NOT NULL
                    AND COALESCE(cost.is_placeholder, FALSE) = FALSE
                    AND UPPER(COALESCE(cost.supplier, '')) NOT LIKE '%%AUTO_TEMPLATE%%'
                    AND UPPER(COALESCE(cost.supplier, '')) NOT LIKE '%%OPERATOR_TRUSTED_COST%%'
                    AND COALESCE(cost.cost_price, cost.unit_cost, 0) > 0
                ) THEN TRUE
                ELSE FALSE
            ...
            CASE
                ...
                WHEN COALESCE(cost.is_placeholder, FALSE) OR UPPER(COALESCE(cost.supplier, '')) LIKE '%%AUTO_TEMPLATE%%' THEN 'placeholder'
                WHEN LOWER(COALESCE(cost.cost_source, '')) = 'supplier_confirmed' THEN 'supplier_confirmed'
                WHEN (
                    LOWER(COALESCE(cost.cost_source, '')) = 'manual_upload'
                    AND cost.upload_id IS NOT NULL
                    AND COALESCE(cost.is_placeholder, FALSE) = FALSE
    ...
    op.execute("CREATE INDEX IF NOT EXISTS ix_mart_sku_daily_account_stat_date ON mart_sku_daily (account_id, stat_date)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_mart_sku_daily_account_nm_stat_date ON mart_sku_daily (account_id, nm_id, stat_date)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_mart_sku_daily_account_sku_stat_date ON mart_sku_daily (account_id, sku_id, stat_date)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_mart_stock_daily_account_sku_stat_date ON mart_stock_daily (account_id, sku_id, stat_date)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_wb_ad_stats_daily_account_nm_stat_date ON wb_ad_stats_daily (account_id, nm_id, stat_date)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_wb_realization_rows_account_nm_rr_date ON wb_realization_report_rows (account_id, nm_id, rr_date)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_data_quality_issues_account_severity_resolved ON data_quality_issues (account_id, severity, resolved_at)")
    op.execute("CREATE INDEX IF NOT EXISTS ix_action_recommendations_account_status_priority ON action_recommendations (account_id, status, priority)")
