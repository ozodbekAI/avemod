...
export const Route = createFileRoute("/_authenticated/data-fix")({
  ...
  validateSearch: (s: Record<string, unknown>): DataFixSearch => ({
    financial_final_blocker:
      s.financial_final_blocker === true || s.financial_final_blocker === "true"
        ? true
        : s.financial_final_blocker === false ||
            s.financial_final_blocker === "false"
          ? false
          : undefined,
    only_open:
      s.only_open === true || s.only_open === "true"
        ? true
        : s.only_open === false || s.only_open === "false"
          ? false
          : undefined,
    severity:
      typeof s.severity === "string"
        ? s.severity
        : Array.isArray(s.severity)
          ? (s.severity as unknown[]).join(",")
          : undefined,
...
const CODE_META: Record<string, CodeMeta> = {
  missing_manual_cost: {
    simple_reason:
      "Для части карточек не загружена реальная себестоимость поставщика — прибыль считается на оценочной.",
    first_action:
      "Откройте экран «Себестоимость» и загрузите xlsx с колонками nm_id, sku, cost.",
    how_to_fix: [
  ...
  manual_cost_ambiguous_match: {
    simple_reason:
      "Один файл себестоимости подходит к нескольким SKU — система не знает, какой выбрать.",
    ...
    success_check: [
      "Конфликт пропал из вкладки.",
      "Себестоимость присвоилась нужному SKU.",
    ],
    next_screen_path: "/costs",
  ...
  manual_cost_unresolved_sku: {
    simple_reason:
      "В файле себестоимости есть строки, которые не привязались ни к одному SKU каталога.",
    first_action:
      "Скачайте список непривязанных строк и проверьте nm_id / sku.",
    how_to_fix: [
  ...
  sale_without_finance: {
    simple_reason:
      "Продажа уже произошла, но финансовая строка от WB по ней ещё не пришла. Это системная задержка сверки, а не ручная задача продавца.",
    first_action:
      "Ничего вручную исправлять не нужно: система дождётся финансового отчёта WB и повторит сверку.",
    how_to_fix: [
    ...
    owner_message:
      "Пользователь не должен менять продажи или суммы вручную. Исправление происходит через синхронизацию финансов WB и сопоставление строк.",
    wait_or_fix_hint:
      "Если продажа свежая, это нормальная задержка финансов WB.",
    next_screen_path: "/finance",
    next_screen_label: "Открыть финансы",
  },
  finance_reconciliation_mismatch: {
    simple_reason:
      "Это не ошибка, которую продавец должен чинить вручную. Система сравнила операционную выручку с финансовым отчётом WB и нашла расхождение.",
    first_action:
      "Сначала проверьте, завершена ли загрузка финансового отчёта WB за этот период. Если отчёт свежий или неполный — это задача синхронизации, а не ручной правки пользователя.",
    how_to_fix: [
    ...
    owner_message:
      "Продавец может только проверить период и запросить повторную синхронизацию. Причина исправляется в системной загрузке данных: импорт финансов WB, сопоставление продаж с отчётом, дедупликация и формулы сверки.",
    wait_or_fix_hint:
      "Если данные свежие или период WB ещё открыт, корректнее подождать закрытия отчёта и следующей синхронизации.",
    next_screen_path: "/finance",
    next_screen_label: "Проверить сверку",
  },
  finance_without_sale: {
    simple_reason:
      "В финансовом отчёте WB есть строка, для которой у нас ещё не сопоставилась соответствующая продажа. Это системная задача синхронизации и сопоставления.",
    first_action:
      "Ничего вручную исправлять не нужно: система повторит загрузку продаж и заказов, затем заново сопоставит строки.",
    how_to_fix: [
    ...
    owner_title: "Это системная сверка",
    owner_message:
      "Пользователь не должен вручную подгонять продажи под финансовый отчёт. Исправление происходит через системную синхронизацию и сопоставление.",
    next_screen_path: "/finance",
    next_screen_label: "Открыть Финансы",
  },
  unmatched_sku: {
    simple_reason:
      "В продажах, остатках или себестоимости есть строка с товарным идентификатором, который система не смогла сопоставить с карточкой каталога. Из-за этого деньги или остатки могут попасть не в тот товар.",
    first_action:
      "Начните с экрана «Себестоимость» → блок «Перепривязать SKU»: найдите строку по nm_id, артикулу продавца или баркоду и запустите перепривязку.",
    how_to_fix: [
  ...
  expense_unclassified: {
    simple_reason:
      "Расходная строка WB пришла, но категория расхода не распознана автоматически.",
    first_action:
      "Откройте «Деньги» → блок «Расходы без категории» и присвойте категорию.",
    how_to_fix: [
  ...
  stocks_task_failed: {
    simple_reason:
      "Задача синхронизации остатков упала — текущий остаток не подтверждён.",
    first_action:
      "Откройте «Админка» → «Синхронизация» и перезапустите загрузку остатков.",
    how_to_fix: [
  ...
  no_main_photo: {
    simple_reason:
      "У карточки отсутствует главное фото — товар плохо ранжируется и слабо конвертит.",
    first_action:
      "Откройте фотостудию — мы создадим проект и подтянем доступные исходники WB.",
    how_to_fix: [
    ...
    success_check: [
      "В фотостудии есть одобренная версия.",
      "Главное фото обновлено в WB.",
    ],
  ...
  few_photos: {
    simple_reason:
      "Слишком мало фотографий — покупатели не видят товар с разных ракурсов.",
    ...
    success_check: [
      "Количество фото в карточке выросло.",
      "Фотостудия показывает «Готово».",
    ],
    next_screen_path: "/photo-studio",
    next_screen_label: "Открыть фотостудию",
  },
  duplicate_images: {
    simple_reason:
      "Несколько изображений карточки дублируют друг друга — это снижает доверие.",
  ...
  main_image_issue: {
    simple_reason:
      "С главным изображением что-то не так — низкое качество, неподходящий фон или композиция.",
  ...
  title_missing: {
    simple_reason:
      "У карточки нет названия — покупатель и поиск WB не понимают, что продаётся.",
  ...
  title_too_short: {
    ...
    success_check: [
      "Название стало информативным.",
      "Проверка качества больше не показывает проблему title.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  title_too_long: {
    ...
    first_action:
      "Сократите название до товара и нескольких важных характеристик.",
    how_to_fix: [
      "Откройте карточку товара.",
    ...
    success_check: [
      "Название читается без обрезки.",
      "Блокер title исчез после повторной проверки.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  title_repeated_words: {
    ...
    success_check: [
      "Название стало естественным.",
      "Повторная проверка не показывает проблему title.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  title_excessive_punctuation_caps: {
    simple_reason:
      "Название выглядит агрессивно из-за caps lock или лишней пунктуации.",
    ...
    success_check: [
      "Название выглядит аккуратно.",
      "Проверка больше не показывает проблему оформления title.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  title_equals_vendor_code: {
    simple_reason:
      "Название совпадает с артикулом продавца и не объясняет товар покупателю.",
    ...
    success_check: [
      "Название больше не совпадает с vendor_code.",
      "Проверка качества стала чище.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  description_missing: {
    simple_reason:
      "Описание отсутствует — покупатель не видит состав, назначение и ограничения товара.",
  ...
  description_too_short: {
    simple_reason:
      "Описание слишком короткое и не закрывает вопросы покупателя.",
    ...
    success_check: [
      "Описание стало содержательным.",
      "Проверка больше не показывает description_too_short.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  description_duplicates_title: {
    simple_reason:
      "Описание повторяет название и не добавляет полезной информации.",
    ...
    success_check: [
      "Описание отличается от названия.",
      "Блокер description исчез.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  description_no_useful_details: {
    ...
    success_check: [
      "Описание содержит конкретные детали.",
      "Повторная проверка не показывает проблему description.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  characteristics_missing: {
    simple_reason:
      "У карточки нет характеристик — фильтры и сравнение товара работают хуже.",
    ...
    success_check: [
      "Характеристики появились.",
      "Блокер characteristics исчез.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  characteristic_value_empty: {
    ...
    first_action:
      "Заполните пустое значение или удалите лишнюю характеристику.",
    how_to_fix: [
      "Откройте карточку товара.",
    ...
    success_check: [
      "Пустых характеристик не осталось.",
      "Проверка characteristics стала чище.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  high_value_characteristics_missing: {
    simple_reason:
      "Не заполнены базовые характеристики вроде цвета, материала, размера или комплектации.",
    first_action:
      "Добавьте ключевые характеристики, которые важны для выбора товара.",
    how_to_fix: [
      "Откройте карточку товара.",
    ...
    success_check: [
      "Базовые характеристики заполнены.",
      "Проверка качества больше не показывает этот блокер.",
    ],
    next_screen_path: "/products",
    next_screen_label: "Открыть товары",
  },
  media_no_images: {
    simple_reason:
      "У карточки нет фотографий — она не готова к нормальной продаже.",
    ...
    success_check: [
      "У карточки появилось главное фото.",
      "Проверка media больше не критичная.",
    ],
    next_screen_path: "/photo-studio",
    next_screen_label: "Открыть фотостудию",
  },
  media_too_few_images: {
    simple_reason:
      "Фотографий мало — покупатель не видит товар с разных сторон.",
    ...
    success_check: [
      "Фото стало не меньше трёх.",
      "Проверка media больше не показывает media_too_few_images.",
    ],
    next_screen_path: "/photo-studio",
    next_screen_label: "Открыть фотостудию",
  },
  media_duplicate_urls: {
    simple_reason:
      "В карточке повторяются фото, которые не дают покупателю новой информации.",
  ...
  media_invalid_url: {
    simple_reason:
      "В карточке есть некорректная ссылка на изображение — возможно, медиа синхронизировались не полностью.",
    ...
    success_check: [
      "Ссылки на фото корректны.",
      "media_invalid_url исчез после повторной проверки.",
    ],
    next_screen_path: "/photo-studio",
    next_screen_label: "Открыть фотостудию",
  },
  price_jump: {
    simple_reason:
      "Платформа зафиксировала резкое изменение цены. Это не ошибка данных — это сигнал перепроверить решение.",
    first_action:
      "Откройте страницу цен и проверьте: планировалось ли такое изменение и корректна ли минимальная безопасная цена.",
    how_to_fix: [
    ...
    owner_message:
      "Платформа только показывает резкое изменение и предлагает проверить его. Никакие цены не меняются автоматически.",
    next_screen_path: "/pricing",
    next_screen_label: "Проверить цену",
  },
  missing_cost_blocks_profit: {
    simple_reason:
      "Без себестоимости платформа не может посчитать прибыль и маржу по этой карточке.",
    ...
    success_check: [
      "У карточки появилась подтверждённая себестоимость.",
      "Расчёт прибыли и маржи разблокирован.",
    ],
    owner_kind: "user",
    next_screen_path: "/costs",
...
const EXACT_OWNER: Record<
  string,
  Pick<CodeMeta, "owner_kind" | "owner_title" | "owner_message">
> = {
  latest_stocks_not_completed: {
    owner_kind: "system",
    owner_title: "Ответственный: система / администратор",
    owner_message:
      "Это не ручная правка цифр. Нужно дождаться завершенной загрузки остатков или перезапустить синхронизацию.",
  },
  failed_sync_domains: {
    owner_kind: "admin",
    owner_title: "Ответственный: система / администратор",
    owner_message:
      "Проблема в загрузке источников данных. Пользователь не должен менять суммы вручную; нужно перезапустить sync или открыть админ-разбор.",
  },
  open_blocking_dq_issues: {
    owner_kind: "aggregate",
    owner_title: "Это сводный блокер",
    owner_message:
      "Его нужно раскрывать на конкретные задачи ниже: себестоимость, SKU, расходы, синхронизация или админ-разбор.",
  },
  finance_reconciliation_mismatch: {
    owner_kind: "system",
    owner_title: "Ответственный: система / администратор",
    owner_message:
      "WB финансовые факты нельзя подгонять вручную. Сначала повторяется загрузка и сверка, затем при необходимости подключается администратор.",
  },
  sale_without_finance: {
    owner_kind: "system",
    owner_title: "Ответственный: система",
    owner_message:
      "Чаще всего это задержка финансового отчета WB. Пользователь ничего вручную не исправляет.",
  },
  finance_without_sale: {
    owner_kind: "system",
    owner_title: "Ответственный: система",
    owner_message:
      "Это сопоставление отчета WB с продажами/заказами. Исправление идет через синхронизацию и re-check.",
  },
  ads_overallocated_to_profitability: {
    owner_kind: "admin",
    owner_title: "Ответственный: администратор",
    owner_message:
      "Это риск формулы или маппинга рекламы. Пользователь не должен вручную менять рекламные расходы.",
  },
};
...
function issueLookupCodesForBlocker(code?: string | null): string[] {
  ...
  return Array.from(
    new Set([...(BLOCKER_TO_ISSUE_CODES[normalized] ?? []), normalized]),
  );
}

function issueForBlocker(
  blocker: MDataBlocker,
  issueByCode: Map<string, DataQualityIssue>,
...
function ownershipForCode(
  ...
  if (
    ...
    return {
      ...
      owner_title: "Ответственный: система / администратор",
      owner_message:
        "Пользователь не должен вручную менять суммы. Сначала проверяется синхронизация и импорт, затем при необходимости администратор разбирает правила загрузки WB и сверки данных.",
    };
  }
  if (
    ...
    return {
      ...
      owner_message:
        "Эта проблема закрывается вводом реальных бизнес-данных или заполнением карточки в кабинете WB.",
    };
  }
  if (
    ...
    return {
      ...
      owner_message:
        "Оператор проверяет проблему и запускает привязку или повторную синхронизацию. Если через текущие экраны она не закрывается, администратор проверяет правила сопоставления и импорта.",
    };
  }
  return {
    owner_kind: "mixed",
    ...
    owner_message:
      "Сначала откройте рекомендованный экран. Если там нет понятного действия для исправления, передайте проблему администратору на проверку импорта и сопоставления данных.",
  };
}
...
function routeInstruction(path: string, code?: string | null): string {
  ...
  if (path.startsWith("/costs"))
    return "Экран: Себестоимость. Загрузите нужный файл, проверьте предпросмотр и подтвердите импорт.";
  if (path.startsWith("/finance"))
    return "Экран: Финансы WB. Сначала проверьте сверку и финансовый отчёт, затем запустите повторную синхронизацию или передайте проблему администратору.";
  if (path.startsWith("/money"))
    return "Экран: Деньги. Откройте детализацию расходов или рекламы и найдите строку, из-за которой появилась проблема.";
  if (path.startsWith("/data-fix"))
    return "Экран: Починка данных. Начните с первого критичного блокера в списке ниже и выполните его шаги.";
  if (path.startsWith("/products"))
    return "Экран: Товары. Откройте карточку и заполните рекомендованное поле контента в кабинете WB.";
  if (path.startsWith("/photo-studio"))
    return "Экран: Фотостудия. Подготовьте нужное изображение, подтвердите его и загрузите в кабинет WB.";
  if (path.startsWith("/admin") || path.startsWith("/dashboard"))
    return "Экран: Админка → Синхронизация. Найдите проблемный домен и запустите повторную загрузку.";
...
function isSystemHandledBlocker(blocker: MDataBlocker): boolean {
  ...
  const backendNature = (blocker as any).issue_nature as
    | string
    | null
    | undefined;
  if (backendOwner === "system" || backendOwner === "admin") return true;
  if (
    backendFix === "system_only" ||
    backendFix === "admin_only" ||
    backendFix === "wait_for_sync"
  )
    return true;
  if (
    backendNature === "system_check" ||
    backendNature === "sync_waiting" ||
    backendNature === "finance_investigation"
  )
    return true;
  const owner = ownershipForCode(
    String(blocker.code || "").toLowerCase(),
  ).owner_kind;
...
function normalizeNextScreenPath(code: string, path?: string | null): string {
  ...
  if (resolved === "/costs") {
    ...
    if (
      code === "unmatched_sku_detected" ||
      code === "unmatched_sku" ||
      code.includes("unresolved_sku")
    ) {
...
function metaFor(b: MDataBlocker): CodeMeta {
  ...
  return {
    simple_reason:
      (staticFirst
        ? fb?.simple_reason || b.simple_reason
        : b.simple_reason || fb?.simple_reason) ||
      b.business_impact ||
      "Проверьте детали ниже.",
    first_action:
      (staticFirst
        ? fb?.first_action || b.first_action
        : b.first_action || fb?.first_action) ||
      (b.how_to_fix?.[0] ?? "Откройте экран ниже и следуйте подсказкам."),
    how_to_fix:
      (staticFirst
        ? fb?.how_to_fix
        : b.how_to_fix && b.how_to_fix.length > 0
          ? b.how_to_fix
          : fb?.how_to_fix) ?? [],
    success_check: (staticFirst
      ? fb?.success_check
      : b.success_check && b.success_check.length > 0
        ? b.success_check
        : fb?.success_check) ?? ["Блокер пропал из этого списка."],
    wait_or_fix_hint:
      (staticFirst
        ? (fb?.wait_or_fix_hint ?? b.wait_or_fix_hint)
        : (b.wait_or_fix_hint ?? fb?.wait_or_fix_hint)) ?? undefined,
    owner_kind: ownership.owner_kind,
    owner_title: ownership.owner_title,
    owner_message: ownership.owner_message,
    next_screen_path: normalizeNextScreenPath(
      code,
      staticFirst
        ? fb?.next_screen_path || b.next_screen_path
        : b.next_screen_path || fb?.next_screen_path,
    ),
    next_screen_label:
      (staticFirst
        ? fb?.next_screen_label || b.next_screen_label
        : b.next_screen_label || fb?.next_screen_label) || "Перейти к починке",
  };
}
  ...
  return (
    <section className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 space-y-2">
      ...
      <div className="text-sm leading-relaxed">
        Откройте <b>Себестоимость → Перепривязать SKU</b> и фильтруйте строку по
        этим полям: сначала <b>nm_id</b>, затем <b>SKU</b>, <b>баркод</b> и{" "}
        <b>артикул продавца</b>.
      </div>
      {rows.length > 0 ? (
        <div className="grid gap-2">
          {rows.map((row, index) => {
            ...
            const vendorCode = valueFromAny(row, [
              "vendor_code",
              "vendorCode",
              "article",
              "sa_name",
            ]);
            const source = valueFromAny(row, [
              "source",
              "source_table",
              "source_module",
              "source_name",
            ...
            return (
              <div
                key={index}
                className="grid grid-cols-2 md:grid-cols-5 gap-2 rounded border bg-background/70 p-2 text-xs"
              >
                <div>
                  <span className="text-muted-foreground">nm_id</span>
                  <div className="font-mono">{nmId ?? "—"}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">SKU</span>
                  <div className="font-mono">{skuId ?? "—"}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Баркод</span>
                  <div className="font-mono">{barcode ?? "—"}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Артикул</span>
                  <div className="font-mono">{vendorCode ?? "—"}</div>
                </div>
                <div>
                  <span className="text-muted-foreground">Источник</span>
                  <div>{source ?? "продажи / остатки / себестоимость"}</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="text-xs text-muted-foreground">
          В ответе API нет конкретной строки-примера. Поэтому начните с общего
          списка «Перепривязать SKU» и сортируйте по последним проблемным
          строкам.
        </div>
      )}
    </section>
...
function GuidedWorkflowCard({
  ...
  return (
    <section
      className={`rounded-md border p-3 ${tone === "primary" ? "border-primary/30 bg-primary/5" : "bg-background"}`}
    >
      <div
        className={`text-[11px] uppercase tracking-wide font-semibold flex items-center gap-1.5 ${tone === "primary" ? "text-primary" : "text-muted-foreground"}`}
      >
        {icon} {title}
      </div>
    ...
    <section className="rounded-md border bg-primary/5 border-primary/25 p-3">
      ...
      <ol className="mt-2 space-y-2 text-sm">
        ...
        <li className="flex gap-2">
          ...
          <span>
            Нажмите <b>{meta.next_screen_label}</b>, если нужно открыть исходный
            экран: {routeInstruction(meta.next_screen_path)}.
          </span>
        </li>
        <li className="flex gap-2">
          <span className="shrink-0 inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary/15 text-primary text-[11px] font-semibold">
          ...
          <span>
            После исправления нажмите <b>Обновить</b> или дождитесь
            автоматической перепроверки. Платформа пересчитает проблему и уберет
            её из списка, если условие выполнено.
          </span>
        </li>
      </ol>
    </section>
...
function BlockerCard({
  ...
  const revenueOnlyBlocked =
    moneyTrust.impact_kind === "blocked_revenue" ||
    ((blocker.affected_amount ?? 0) === 0 &&
  ...
  return (
    <>
      <Card
        ...
        className={`relative overflow-hidden scroll-mt-24 transition-shadow ${highlight ? "ring-2 ring-primary shadow-lg" : ""}`}
      >
          ...
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div className="space-y-1.5 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge
                  variant="outline"
                  className={`text-[10px] uppercase ${style.badge}`}
                >
        ...
        <CardContent className="pl-6 space-y-4">
          {showStats ? (
            <div
              className={`grid grid-cols-2 ${showCurrentRequired ? "sm:grid-cols-4" : "sm:grid-cols-2"} gap-2 text-xs`}
            >
              ...
              <div className="rounded-md border bg-muted/30 px-2.5 py-2">
                ...
                {revenueOnlyBlocked ? (
                  <div className="text-[10px] text-muted-foreground">
                    это не подтвержденная потеря денег
                  </div>
                ) : null}
              </div>
              {showCurrentRequired ? (
                <>
                  <div className="rounded-md border bg-muted/30 px-2.5 py-2">
                    ...
                    <div className="font-semibold text-sm">
                      {formatNumber(blocker.current_value ?? 0)}
                  ...
                  <div className="rounded-md border bg-muted/30 px-2.5 py-2">
                    ...
                    <div className="font-semibold text-sm">
                      {formatNumber(blocker.required_value ?? 0)}
                      {blocker.unit ? ` ${blocker.unit}` : ""}
                    </div>
                  </div>
                </>
          ...
          {m.success_check.length > 0 ? (
            <section className="rounded-md border border-emerald-500/30 bg-emerald-500/5 p-3">
              <div className="text-[11px] uppercase tracking-wide text-emerald-700 dark:text-emerald-300 font-semibold flex items-center gap-1.5">
                <CheckCircle2 className="h-3.5 w-3.5" /> Как понять, что всё
                готово
              </div>
          ...
          {m.wait_or_fix_hint ? (
            <section className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3">
              <div className="text-[11px] uppercase tracking-wide text-amber-700 dark:text-amber-300 font-semibold flex items-center gap-1.5">
                <Hourglass className="h-3.5 w-3.5" /> Когда можно просто
                подождать
              </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {issue &&
            onOpenWorkbench &&
            (blocker as any).can_user_fix_inside_platform !== false ? (
              <Button
                size="sm"
                variant="outline"
                onClick={() => onOpenWorkbench(issue, blocker)}
            ...
            <Button asChild size="sm" variant="default">
              <Link
                {...(linkPropsForPath(
                  (blocker as any).target_href ?? m.next_screen_path,
                ) as any)}
              >
                {(blocker as any).primary_action_label ?? m.next_screen_label}{" "}
                <ArrowRight className="h-3.5 w-3.5 ml-1.5" />
              </Link>
            </Button>
          </div>
          ...
          {blocker.exact_next_endpoint ||
          (blocker.related_endpoints?.length ?? 0) > 0 ? (
...
function SystemStatusCard({
  ...
  return (
    <Card
      id={`blocker-${blocker.code}`}
      className={`scroll-mt-24 border-dashed bg-muted/20 transition-shadow ${
...
function BlockerCalculation({ blocker }: { blocker: MDataBlocker }) {
  const inputs = Array.isArray(blocker.calculation_inputs)
    ? blocker.calculation_inputs
    : [];
  const endpoints =
    Array.isArray(blocker.source_endpoints) &&
    blocker.source_endpoints.length > 0
      ? blocker.source_endpoints
      : [
          blocker.exact_next_endpoint,
          ...(blocker.related_endpoints ?? []),
        ].filter(Boolean);
  if (
    ...
    <section className="rounded-md border bg-muted/25 p-3">
          ...
          {inputs.map((row, i) => (
            ...
            >
              <div className="text-[11px] text-muted-foreground">
                {row.label ?? "Показатель"}
              </div>
              <div className="font-semibold text-sm tabular-nums">
                {formatCalculationValue(row.value, row.unit)}
              </div>
              {row.source ? (
      ...
      {endpoints.length > 0 ? (
        <details className="mt-2 text-[11px] text-muted-foreground">
          <summary className="cursor-pointer select-none">
            Технические источники для поддержки
          </summary>
          <div className="mt-1 space-y-1">
            {endpoints.slice(0, 6).map((endpoint, i) => (
              <div
                key={i}
                className="font-mono text-[10px] rounded bg-background/80 px-2 py-1 break-words"
              >
                {endpoint}
              </div>
            ))}
...
function DataFixPage() {
  ...
  const blockersQ = useQuery({
    ...
    queryFn: () =>
      fetchDataBlockers({
        ...
        dateFrom,
        dateTo,
      }) as Promise<MDataBlockersResponse>,
  });
  ...
  const dqSummaryQ = useQuery<DataQualityIssueSummaryResponse | null>({
    ...
    queryFn: async ({ signal }) => {
      try {
        return await api<DataQualityIssueSummaryResponse>(
          API_ENDPOINTS.dq.summary,
          {
            query: buildBizQuery({ accountId: activeId, dateFrom, dateTo }),
            signal,
          },
  ...
  const issueCodesForLookup = useMemo(() => {
  ...
  const blockerIssuesQ = useQuery<DataQualityIssuesPage | null>({
    queryKey: ["dq-issues-for-data-fix-codes", activeId, issueCodesForLookup],
    enabled: !!activeId && issueCodesForLookup.length > 0,
    staleTime: 30_000,
    retry: false,
    queryFn: async ({ signal }) => {
      try {
  ...
  const issueByCode = useMemo(() => {
    ...
    for (const issue of blockerIssuesQ.data?.items ?? []) {
  ...
  const dqTotalIssues: number = Number(
    dq?.financial_final_blockers_total ??
      dq?.blocking_open_issues_total ??
      dq?.open_issues_total ??
      NaN,
  );
  const dqBlockerTypes: number = (() => {
    const byGroup = dq?.by_group ?? dq?.by_issue_type ?? null;
    if (byGroup && typeof byGroup === "object")
      return Object.keys(byGroup).length;
    return Number.NaN;
  })();
  const financialFinalBlockers = Number.isFinite(dqTotalIssues)
    ? dqTotalIssues
    : (data?.data_quality_summary?.financial_final_blockers_total ??
      blockers.length);
  ...
  const heroTone: "danger" | "warning" | "success" =
    overallState === "data_blocked" || blockersCount > 0
      ? "danger"
      : overallState === "accepted_with_warnings" ||
          warningsCount > 0 ||
  ...
  }, [focusCode, blockers.length, systemBlockers.length, warnings.length]);
  ...
  const matchesCategoryFn = (b: MDataBlocker, cat: string): boolean => {
    ...
    switch (cat) {
      ...
      case "missing_data":
        return (
          /missing|no_|not_set|empty|coverage_below/.test(code) ||
          b.priority === "critical"
        );
      case "cost":
      ...
      case "sku_map":
        return (
          /sku|barcode|vendor|chrt|unmatched|mapping/.test(code) &&
          !/cost/.test(code)
        );
      case "expense":
  ...
  const summaryCounts = useMemo(() => {
    ...
    return {
      ...
      canFixHere: all.filter(
        (b) =>
          matchesCategoryFn(b, "can_fix_here") && !isSystemHandledBlocker(b),
      ).length,
      needsSync: all.filter((b) =>
  ...
  const manualProblemWord = pluralRu(
    blockersCount,
    "проблема",
    "проблемы",
    "проблем",
  );
  const manualProblemRelation =
    manualProblemWord === "проблема"
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              ...
              disabled={blockersQ.isFetching}
            >
              <RefreshCw
                className={`h-4 w-4 mr-1.5 ${blockersQ.isFetching ? "animate-spin" : ""}`}
              />
              Обновить
            </Button>
          </div>
      ...
      <div className="space-y-6">
        ...
        {activeId && (
          <OperationalFinalBanner
            operational_trusted={
              (healthQ.data as any)?.operational_trusted ??
              (healthQ.data as any)?.business_trusted ??
              null
            }
        ...
        {blockersQ.isLoading ? (
          ...
          <Alert
            variant={heroTone === "danger" ? "destructive" : "default"}
            className={
              heroTone === "warning" ? "border-warning/40 bg-warning/5" : ""
            }
          >
            {blockersCount > 0 ? (
            ...
            <AlertDescription className="space-y-1">
              <div>
                {blockersCount > 0
                  ? `Есть ${blockersCount} ${manualProblemWord}, ${manualProblemRelation} вручную. Откройте список ниже и выполните шаги.`
                  : systemBlockersCount > 0
                    ...
                    : (overallMessage ??
                      `Есть ${warningsCount} предупреждений. Их можно закрыть, когда будет время.`)}
              </div>
          ...
          <div className="text-xs text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
            <span>
              ...
              <b className="text-foreground">
                {(healthQ.data as any).trust_state
                  ? humanizeBusinessStatus((healthQ.data as any).trust_state)
                      .label
                  : "—"}
              </b>
            </span>
            <span>
              Финальная сверка:{" "}
              <b className="text-foreground">
                {(healthQ.data as any).financial_final ? "готова" : "не готова"}
              </b>
            </span>
            {(healthQ.data as any).open_issues_total != null ? (
              <span>
                Открытых проблем:{" "}
                <b className="text-foreground">
                  {(healthQ.data as any).open_issues_total}
                </b>
              </span>
            ) : null}
            <Link
        ...
        <section className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline">
              {filteredBlockers.length} из {sortedBlockers.length}
            </Badge>
          </div>
          <p className="text-sm text-muted-foreground">
            Здесь только то, что пользователь реально может закрыть:
            себестоимость, SKU, категории расходов или другие бизнес-данные.
          </p>
        ...
        {systemBlockers.length > 0 ? (
          <section className="space-y-3">
            ...
            <Alert className="border-warning/40 bg-warning/5">
              ...
              <AlertDescription>
                Эти проблемы закрываются через sync, отчет WB, формулу или
                админскую проверку. Пользователь только смотрит статус,
                запускает повторную проверку или передает в разбор.
              </AlertDescription>
            </Alert>
            {isMobile ? (
              <div className="space-y-2.5">
        ...
        <div className="text-[11px] text-muted-foreground text-center pt-2">
          ...
          {data?.meta?.generated_at
            ? ` · обновлено ${formatGeneratedAt(data.meta.generated_at)}`
            : ""}
        </div>
      </div>
      <DataFixWorkbench
